        ...
        <div class="max-w-4xl mx-auto px-4 py-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                <div class="p-6 sm:p-8">
                    ...
                    @if($deduplicatedLinks->count() > 0 || $artist->spotify_artist_id || $artist->apple_music_artist_id || $artist->wikipedia_url)
                        <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                @if($artist->apple_music_artist_id)
                                    <a href="https://music.apple.com/us/artist/{{ $artist->apple_music_artist_id }}"
                                       target="_blank"
                                       rel="noopener noreferrer"
                                ...
                                @if($artist->wikidata_qid)
                                    <a href="https://www.wikidata.org/wiki/{{ $artist->wikidata_qid }}"
                                       target="_blank"
                                       rel="noopener noreferrer"
                                ...
                                @foreach($deduplicatedLinks as $link)
                                    @if(!in_array($link->type, ['spotify', 'apple_music', 'website']))
                                        <a href="{{ $link->url }}"
                                           target="_blank"
                                           rel="noopener noreferrer"
                                           class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors">
                                            @switch($link->type)
                                                ...
                                                @case('bandcamp') Bandcamp @break
                                                @case('soundcloud') SoundCloud @break
                                                @case('deezer') Deezer @break
                                                @case('reddit') Reddit @break
                    ...
                    @if(count($albumsByType))
                        <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                            @foreach($albumsByType as $group)
                                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 @if(!$loop->first) mt-8 @endif">{{ $group['label'] }}</h2>
